...
1. `POST /ingest` — accepts a PDF, saves it to `temp_uploads/`, and queues a background
   task (`run_ingestion_logic` in `backend/src/services/ingestion.py`)
...
4. `POST /chat` — runs the same hybrid search, assembles a context-only prompt, and
   sends it to the llama.cpp container's `/completion` endpoint (`backend/src/services/chat.py`)
5. `GET /health` — liveness check, also reports the detected GPU/CPU device

**Key layers:**
- `backend/src/core/` — settings (Pydantic), logger, custom exceptions (`RAGError`
  hierarchy), `LlamaCppClient` (HTTP client for the llama.cpp container),
  `VRAMScheduler` (GPU time-sharing, see below)
...
- `backend/src/services/` — business logic for ingestion, search, and chat (called from
  routes)
...
(`backend/src/core/vram_scheduler.py`) enforces strict time-sharing via one `asyncio.Lock`:
...
and `get_vram_scheduler()` in `backend/src/api/dependencies.py` are `lru_cache`-backed
...
watcher is started/stopped via the FastAPI lifespan in `backend/src/main.py`.
